...
const WEBKIT_SMOKE_TAG = "@webkit-smoke";
...
async function waitForDurableMarkdown(page: Parameters<typeof openEditor>[0], markdown: string) {
  await expect.poll(async () => {
    ...
    // WebKit versions differ in optional OPFS support. The smoke contract only
...
test(`${WEBKIT_SMOKE_TAG} starts and accepts editor typing`, async ({ page }) => {
  const editor = await openEditor(page);
  await editor.pressSequentially("WebKit typing smoke");
  await expect(editor).toHaveText("WebKit typing smoke");
});

test(`${WEBKIT_SMOKE_TAG} keeps a note after a durable reload`, async ({ page }) => {
  const editor = await openEditor(page);
  const markdown = "WebKit persistence smoke";
...
test(`${WEBKIT_SMOKE_TAG} imports and exports Markdown`, async ({ page }) => {
  const editor = await openEditor(page);
  const markdown = "# WebKit Markdown\n\nPortable content.";
  await page.locator('input[type="file"][accept*="markdown"]').setInputFiles({
    name: "webkit-smoke.md",
    mimeType: "text/markdown",
    buffer: Buffer.from(markdown),
  });
  await expect(editor.locator("h1")).toHaveText("WebKit Markdown");
  await waitForDurableMarkdown(page, markdown);
  ...
  expect(exported).toContain("# WebKit Markdown");
  expect(exported).toContain("Portable content.");
});

test(`${WEBKIT_SMOKE_TAG} persists a selected theme`, async ({ page }) => {
  const editor = await openEditor(page);
