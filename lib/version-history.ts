...
function parseVersions(raw: string | null): DocumentVersion[] {
  ...
  try {
    ...
    return values.flatMap((value) => {
      ...
      return [{ id: candidate.id, markdown: candidate.markdown, createdAt: candidate.createdAt as number }];
...
export function recordDocumentVersion(
  ...
  try {
    ...
    if (!options.force && newest && now - newest.createdAt < CHECKPOINT_INTERVAL_MS) return true;
