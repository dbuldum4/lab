...
persistence = Path("lib/editor-persistence.ts")
...
for old, new in [
    ...
    if old not in text:
        raise SystemExit(f"missing persistence anchor: {old[:100]}")
    text = text.replace(old, new, 1)
persistence.write_text(text)

tests = Path("lib/editor-persistence.test.ts")
...
sessions = Path("lib/document-sessions.ts")
text = sessions.read_text()
old = '} from "./local-vault.ts";\n\nconst SESSION_KEY_PREFIX'
new = '} from "./local-vault.ts";\nimport { clearDocumentVersions } from "./version-history.ts";\n\nconst SESSION_KEY_PREFIX'
if old not in text:
    raise SystemExit("document sessions import anchor missing")
text = text.replace(old, new, 1)
old = '  await deleteLocalDocument(normalized);\n  return deleteDocumentSession(normalized);'
new = '  if (!clearDocumentVersions(normalized)) {\n    throw new Error("Local version history could not be deleted.");\n  }\n  await deleteLocalDocument(normalized);\n  return deleteDocumentSession(normalized);'
if old not in text:
    raise SystemExit("document purge anchor missing")
...
for old, new in replacements:
    if old not in text:
        raise SystemExit(f"missing editor patch anchor: {old[:140]}")
    text = text.replace(old, new, 1)

old = '    if (palette?.mode === "sessions") {\n      const activeSession = sessions[selected];'
new = '    if (palette?.mode === "history") {\n      const activeVersion = historyVersions[selected];\n      documentElement.setAttribute("aria-expanded", "true");\n      documentElement.setAttribute("aria-controls", PALETTE_ID);\n      if (activeVersion) documentElement.setAttribute("aria-activedescendant", `${PALETTE_ID}-history-${activeVersion.id}`);\n      else documentElement.removeAttribute("aria-activedescendant");\n      return;\n    }\n    if (palette?.mode === "sessions") {\n      const activeSession = sessions[selected];'
if old not in text:
    raise SystemExit("history aria anchor missing")
text = text.replace(old, new, 1)
old = '  }, [editor, filtered, palette, selected, sessions]);'
new = '  }, [editor, filtered, historyVersions, palette, selected, sessions]);'
if old not in text:
    raise SystemExit("aria dependency anchor missing")
text = text.replace(old, new, 1)
...
'''
if old not in text:
    raise SystemExit("slash selection reset anchor missing")
